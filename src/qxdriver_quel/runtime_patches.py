...
"""Runtime patch hooks for quelware integration."""
...
def _is_quelware_0_10_or_later() -> bool:
    ...
    try:
        installed = version("quel_ic_config")
...
def _patch_sock_file_lockkeeper(sock_module: Any) -> None:
    """Patch socket file-lock keeper to in-memory lock behavior."""
    ...
    if file_lock_keeper_cls is None or abstract_lock_keeper_cls is None:
        return
    if getattr(file_lock_keeper_cls, "__qxdriver_lockfile_patch_applied__", False):
        return
    ...
    file_lock_keeper_cls.__qxdriver_lockfile_patch_applied__ = True


def _patch_coap_file_lock_client(coap_module: Any) -> None:
    """Patch CoAP file-lock client to in-memory lock behavior."""
    ...
    if file_lock_client_cls is None or abstract_coap_client_cls is None:
        return
    if getattr(file_lock_client_cls, "__qxdriver_lockfile_patch_applied__", False):
        return
    ...
    file_lock_client_cls.__qxdriver_lockfile_patch_applied__ = True


def _apply_filelock_patch() -> None:
    """Apply lockfile-disabling monkey patch when quelware 0.10+ is installed."""
...
def _apply_linkup_patch() -> None:
    """Patch LinkupFpgaMxfe default background noise threshold when available."""
    try:
        from quel_ic_config import LinkupFpgaMxfe
    except ImportError:
        return
    LinkupFpgaMxfe._DEFAULT_BACKGROUND_NOISE_THRESHOLD_AT_RECONNECT = 100000  # type: ignore[attr-defined]


def apply_runtime_patches() -> None:
    """Apply runtime patches required by qubex integrations."""
    _apply_filelock_patch()
    _apply_linkup_patch()
